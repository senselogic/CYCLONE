...
func ( error_message * ERROR_MESSAGE ) Set(
    text string,
    error_ error
    )
{
...
func ( error_message * ERROR_MESSAGE ) SetText(
    text string
    )
{
    error_message.Text = text;
...
func ( error_message * ERROR_MESSAGE ) SetError(
    error_ error
    )
{
    error_message.Text = "";
    ...
    else if ( IsPostgresqlDatabase )
    {
        SqlDatabase, error_
            = sql.Open(
                  ...
                  + " password="
                  + DatabasePassword
                  + GetConditionalText( DatabaseName != "", " dbname=" + DatabaseName )
                  + " sslmode=disable"
                  );
    }
    ...
    )
{
    ...
    if ( ParseArguments( &error_message )
         && OpenDatabase( &error_message )
         && ExecuteScripts( &error_message )
         && CloseDatabase() )
    {
        fmt.Println( "Success." );
    }
